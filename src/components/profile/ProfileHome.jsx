...
export default function ProfileHome() {
    ...
    useEffect(()=>{
        const yourVideos = async () =>{
            try{
                const res = await axios.get(`${BASE_URL}/api/fetch/user/videos/${params.userid}`)
    ...
    if(isLoading){
      return(
        ...
        <p className='mt-4' style={{fontSize:'1rem'}}>
          <i className='bi bi-play-btn-fill'></i> {currUser.email === params.userid?<b>Your uploaded videos</b>:<b>Videos for you</b>}
        </p>
  ...
  return (
    <>
      <div className='container-xxl mb-2'>
        ...
        <section  className=' d-flex  flex-row flex-wrap' >
        {!yourVideos.length? <p>No videos</p> :
